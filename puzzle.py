...
def draw_corona():
    ...
    for y in range(10):
        for x in range(8):
            if corona[y][x] > 0:
                cvs.create_image(x*72+60, y*72+60, image=img_corona[corona[y][x]], tag="CORONA")

def check_corona():
    for y in range(10):
        for x in range(8):
            check[y][x] = corona[y][x]
...
def drop_corona():
    ...
    for y in range(8, -1, -1):
        for x in range(8):
            if corona[y][x] != 0 and corona[y+1][x] == 0:
                corona[y+1][x] = corona[y][x]
                corona[y][x] = 0
                flg = True
    return flg
...
def set_corona():
    for x in range(8):
        corona[0][x] = random.randint(0, difficulty)
...
def game_main():
    ...
    if index == 0: # タイトルロゴ
        draw_txt("コロナ退治", 312, 240, 100, "gray", "TITLE")
        ...
        if 24 <= mouse_x and mouse_x < 24+72*8 and 24 <= mouse_y and mouse_y < 24+72*10:
            ...
            if mouse_c == 1:
                mouse_c = 0
                set_corona()
                corona[cursor_y][cursor_x] = tsugi
                tsugi = 0
                index = 2
        cvs.delete("CURSOR")
    ...
    if tsugi > 0:
        cvs.create_image(752, 128, image=img_corona[tsugi], tag="INFO")
    root.after(100, game_main)

root = tkinter.Tk()
root.title("落ち物パズル「コロナ退治」")
...
bg = tkinter.PhotoImage(file="corona_bg.png")
cursor = tkinter.PhotoImage(file="corona_cursor.png")
img_corona = [
    None,
    tkinter.PhotoImage(file="corona1.png"),
    tkinter.PhotoImage(file="corona2.png"),
    tkinter.PhotoImage(file="corona3.png"),
    tkinter.PhotoImage(file="corona4.png"),
    tkinter.PhotoImage(file="corona5.png"),
    tkinter.PhotoImage(file="corona6.png"),
    tkinter.PhotoImage(file="corona_niku.png")
]
